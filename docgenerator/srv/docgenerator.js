...
module.exports = class generateDocument extends cds.ApplicationService {
  init() {
    ...
    this.on("combinedDocGenerate", async (req) => {
      ...
      console.timeLog("doc-gen-time", "After API call");
      let sapEditable = [];
      let customerEditable = [];
      try {
        var filterPayload = [];
        docResponse.data.forEach((item, index) => {
          if (item.defaultValue !== item.parameterValue) {
            filterPayload.push(item);
            item.isCustomerEditable ? customerEditable.push(item) : sapEditable.push(item);
          }
        });
      ...
      const realPayload = req.data.value;
      realPayload.docpayload.csvGuidedBuyingParameters = csvArray;
      realPayload.docpayload.sapEditableParameters = sapEditable;
      realPayload.docpayload.customerEditableParameters = customerEditable;
    
      const gen = new DocGenerator(realPayload, {
          creator: "Doc Generator",
        });
